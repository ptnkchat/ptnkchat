...
var init = (mongo, callback) => {
  getCollectionNames(mongo, currentTables => {
    ...
    let initTable = name => {
      mongo.conn.createCollection(name, () => {
        doneAdd++;
...
var writeToWaitRoom = (mongo, id, gender) => {
  let d = new Date();
  mongo.conn.collection('waitroom').insertOne({uid: +id, gender: gender, time: d.getTime()}, (err) => {
    if (err) {
...
var findInWaitRoom = (mongo, id, callback) => {
  mongo.conn.collection('waitroom').find({uid: +id}).toArray((err, results) => {
    if (err) {
...
var deleteFromWaitRoom = (mongo, id) => {
  mongo.conn.collection('waitroom').deleteMany({uid: +id}, (err) => {
    if (err) {
...
var getListWaitRoom = (mongo, callback) => {
  mongo.conn.collection('waitroom').find().toArray((err, results) => {
    if (err) {
...
var findPartnerChatRoom = (mongo, id, callback) => {
  mongo.conn.collection('chatroom').find({$or:[{id1: +id}, {id2: +id}]}).toArray((err, results) => {
    if (err) {
...
var findInLastTalk = (mongo, id1, id2) => {
  mongo.conn.collection('lasktalk').find({uid: +id1}).toArray((err, results) => {
    if (err) {
